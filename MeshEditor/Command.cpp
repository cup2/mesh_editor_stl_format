...
Vec Command::getNormal(const Vec& dot1, const Vec& dot2, const Vec& dot3) {
	...
	double lengthvec = sqrt(normal.x*normal.x + normal.y*normal.y + normal.z*normal.z);
...
void Command::getArgument(const std::map<std::string, std::string>& args, const std::string& keyName,  double& value) {
...
void Command::getArgument(const std::map<std::string, std::string>& args, const std::string& keyName, Vec& value) {
	auto it = args.find(keyName);
	if (it != args.end()) {
...
void Command::getArgument(const std::map<std::string, std::string>& args, const std::string& keyName, std::string& value) {
	auto it = args.find(keyName);
	if (it != args.end()) {
		value=it->second;
	}
