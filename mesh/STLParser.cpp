...
bool operator&(const STLParser::TypeLine &first,
               const STLParser::TypeLineUnder &second) {
  return static_cast<bool>(static_cast<int>(first) & static_cast<int>(second));
};

STLParser::TypeLineUnder operator|(const STLParser::TypeLine &first,
                                   const STLParser::TypeLine &second) {
  return static_cast<STLParser::TypeLineUnder>(first) |
         static_cast<STLParser::TypeLineUnder>(second);
};

std::ostream &operator<<(std::ostream &fout, Vec vec) {
  fout << " " << std::to_string(vec.x) << " " + std::to_string(vec.y) << " "
       << std::to_string(vec.z);
  return fout;
};
#include <iostream>
  ...
  if (stlout.is_open()) {
    ...
    for (const auto &it : triangleSoup) {
      if (counterVertices % 3 == 0) {
        ...
        stlout << startFacetText_ + " " + normalText_ + " " << it.normal
               << std::endl;
